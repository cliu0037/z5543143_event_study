...
# ----------------------------------------------------------------------------
ROOTDIR = os.path.join(toolkit_config.PRJDIR, 'project1')
...
def get_tics(pth):
    ...
    """
    # <COMPLETE THIS PART>
    tickers_exchanges_dict = {}
    with open(pth, 'r') as file:
        for line in file:
            line = line.strip()
            if line:
                exchange, ticker = line.split('=')
                exchange = ''.join(x for x in exchange if x.isalpha())
                ticker = ''.join(x for x in ticker if x.isalpha())
                tickers_exchanges_dict[ticker.lower()] = exchange.lower()
    return tickers_exchanges_dict
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is not None:
        if not tickers_lst:
            raise ValueError("tickers_lst is an empty list.")
        for ticker in tickers_lst:
            if ticker.lower() not in tic_exchange_dic:
                raise ValueError(f"The ticker {ticker} does not correspond to a key in tic_exchange_dic.")
...
def verify_cols(col_lst=None):
    ...
    if col_lst is not None:
        if not col_lst:
            raise ValueError("col_lst is an empty list.")

        for col in col_lst:
            if col.capitalize() not in COLUMNS:
                raise ValueError(f"The column {col} is not found in COLUMNS.")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for ticker in tickers_lst or tic_exchange_dic.keys():
        ...
        for line in lines:
            ...
            if col_lst is not None:
                filtered_dict = {col: line_dict[col] for col in col_lst if col in line_dict}
            else:
                filtered_dict = line_dict
            filtered_dicts.append(filtered_dict)

        data_dict[ticker] = {'exchange': tic_exchange_dic[ticker], 'data': filtered_dicts}
...
# ----------------------------------------------------------------------------
    ...
    1.  Configuring is done to make sure that the code is portable and works in different settings. By using expressions 
    to define file paths instead of hardcoding the paths, we can ensure that it makes the code more robust and adaptable
    to a variety of environments.
    
    2.  Firstly, we evaluate whether the articles journalists wrote are based on investors' evaluations of those firms.
    It is less likely that the articles are solely based on investors' evaluations of the firms. If the articles were 
    reflecting investors' evaluations, we should expect the sentiment expressed in the articles to align with the 
    subsequent movement of stock returns. However, the observed negative words in the articles causing a decrease in 
    stock returns suggests that journalists may be incorporating additional information beyond investors' evaluations. 
    Therefore, this hypothesis is less likely to be true. 
        On the other hand, we evaluate whether the articles contain valuable information beyond firm fundamentals. The 
    observed negative sentiment in the articles leading to reduction in stock returns shows the articles may contain 
    valuable information beyond firm fundamentals. If the articles were purely based on firm fundamentals, we should not
    expect to see a significant impact on stock returns from the sentiment expressed. The fact that stock returns 
    decrease in the short run with no reversal in the long run indicates that the articles may reflect insights not 
    captured by traditional firm fundamentals. Therefore, this hypothesis is more likely to be true.
        To summarise, the second hypothesis, suggesting that the articles contain valuable information beyond firm 
    fundamentals, is more likely to be true according to the relationship between article sentiment and stock returns we 
    observed.
    
    3.  Given the hypothesis I have chosen is that there is valuable information beyond firm fundamentals, in the short 
    run, we would expect an increase in trading volume following the publication of articles containing more negative 
    sentiment about a firm's performance. The reason is that negative sentiment in the articles could trigger reactions 
    from investors, leading to more trading activity. Investors will interpret the negative news as potential risks or 
    concerns about the firm's future performance, making them adjust their investment positions. This heightened 
    activity could result in higher trading volumes as investors buy or sell shares in response to their perception of 
